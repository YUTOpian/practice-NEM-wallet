...
/* ============================================================
   ローカル署名 (NIS1向け)
   NIS1の /transaction/announce へ送るペイロードは、手動で組み立てるのではなく
   SDK公式ドキュメント記載の方法(attachSignature)で生成する:
     const signature = facade.signTransaction(keyPair, transaction);
     const jsonPayload = facade.transactionFactory.static.attachSignature(transaction, signature);
   以前は data(署名前バイト列)とsignatureを自前でJSON化していたが、
   NISノード側で正しくバイナリを復元できず
   "expected value for property recipient, but none was found" のようなエラーになっていた。
   attachSignature はSDKが正しい形式のペイロードを組み立ててくれるため、
   その結果をそのままPOSTボディとして使う。
============================================================ */
function buildNemAnnouncePayload(tx) {
  const signature = appState.facade.signTransaction(appState.localKeyPair, tx);

  // ------------------------------------------------------------
  // 診断ログ: 送信前にローカルで自己検証し、SDK内部の署名/検証が
  // 自己整合しているかを確認する(問題の切り分け用)。
